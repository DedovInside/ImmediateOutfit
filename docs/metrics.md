...
Финальная версия для защиты синхронизирована с Excel `ImmediateOutfit_AARRR_dataset.xlsx`.
Дашборд показывает две колонки: презентационный demo seed из Excel и live-расчёт из событий SQLite.

## AARRR-метрики из Excel

1. `new_users` — количество новых пользователей.
2. `avg_time_to_result_min` — среднее время до получения результата в минутах.
3. `scenario_completion_rate` — конверсия в завершение сценария.
4. `avg_scenarios` — среднее количество сценариев на пользователя.
5. `retention_d1` — доля пользователей, вернувшихся на следующий день.
6. `retention_d7` — доля пользователей, вернувшихся на 7-й день.
7. `retention_d30` — доля пользователей, вернувшихся на 30-й день.
8. `ai_stylist_share` — доля пользователей, которые использовали AI-стилиста.
9. `avg_session_length_min` — средняя длина сессии.
10. `avg_bot_requests` — среднее количество обращений к боту.
11. `ai_usage_frequency` — среднее количество использований AI.
12. `cac = marketing_spend / new_users` — стоимость привлечения пользователя.

Эти показатели можно заполнить двумя способами:

- `aarrr_dataset` — презентационный сид из Excel для защиты.
- `events` — live-расчёт из событий бота, чтобы реальные пользователи не ломали демо-цифры.

## Дополнительные метрики продукта
...
- `ai_quota_consumed` — успешный AI-ответ, который списал 1 бесплатный запрос.
- `ai_quota_limit_reached` — пользователь дошёл до лимита бесплатных AI-ответов.
- `result_feedback`
- `review_feedback`
